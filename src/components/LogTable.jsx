...
export default function LogTable({ records, onOpen, onDelete, onTranscript, onAudio }) {
  if (!records.length) {
    return (
      <div className="empty">
      ...
      <tbody>
        {records.map((v, i) => {
          ...
          return (
            <tr key={v._id || i}>
              <td>{v._savedAt || "—"}</td>
              <td><strong>{dash(v.lastName)}, {dash(v.firstName)}</strong></td>
              ...
              <td>
                <div className="pill-row">
                  <span className={"pill " + verdictClass(v, comp.incomplete)}>{v._verdict || "NO AUDIO"}</span>
              ...
              <td style={{ textAlign: "right" }}>
                {v._transcript && <button className="btn btn-ghost btn-sm" onClick={() => onTranscript(i)}>Transcript</button>}{" "}
                {v._audioFile && <button className="btn btn-ghost btn-sm" onClick={() => onAudio(i)}>Audio</button>}{" "}
                <button className="btn btn-ghost btn-sm" onClick={() => onOpen(i)}>Open</button>{" "}
